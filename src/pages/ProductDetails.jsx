...
export default function ProductDetails() {
    ...
    const [selected, setSelected] = useState({}); // { [attributeName]: value }
        ...
        <div style={{ display: "flex", gap: 36, padding: "42px 56px" }}>
            ...
            <div style={{ maxWidth: 480 }}>
                ...
                {isSelectable && product.attributes.map(attr => (
                    <div key={attr.id} data-testid={`product-attribute-${attr.name.replace(/\s+/g, "-").toLowerCase()}`} style={{ margin: "25px 0 14px 0" }}>
                                ...
                                <button
                                    ...
                                    data-testid={
                                        selected[attr.name] === item.value
                                            ? `cart-item-attribute-${attr.name.replace(/\s+/g, "-").toLowerCase()}-${attr.name.replace(/\s+/g, "-").toLowerCase()}-selected`
                                            : `cart-item-attribute-${attr.name.replace(/\s+/g, "-").toLowerCase()}-${attr.name.replace(/\s+/g, "-").toLowerCase()}`
                                    }
                                >
                                    {attr.type === "swatch"
                                        ? <span style={{
